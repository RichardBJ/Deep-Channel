...
scheme= xlsread(filename,1,'B2:F6');
isopen= xlsread(filename,1,'B8:F8');
state=1;
...
%% Multichannels
    ...
    while channels<maxchannels
        ...
        samples=int64(0.9*samples); %may be wasting time to go with shorter search
            ...
            if shorty>5
                shorty=0;
                samples=int64(1.1*samples); %we didn't get enough samples so increase and go again.
    ...
    %outcrop=out(int64(2/dt):end,:);
    outcrop=out;
    outcrop(:,1)=outcrop(:,1);
    csvwrite(['astr' num2str(record) '.csv'],outcrop);
%     plot(outcrop(:,1),outcrop(:,2));
